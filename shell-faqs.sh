...
# $? reads the exit status of the last command executed. An exit status of zero means success. Non-zero means an error or anaomalous condition.

echo hello
echo $?    # Exit status 0 returned because the command executed successfully

lskf
echo $?    # Exit status will be non-zero because the command failed to execute
